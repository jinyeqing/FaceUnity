...
public class STRenderer {
    ...
    // The array length is 1 more than the type count cause
    // the STMobileType starts from 1.
    private int[] mMakeupPackageIds = new int[MakeupManager.getMakeupTypeCount() + 1];
    private String[] mMakeupPaths = new String[MakeupManager.getMakeupTypeCount() + 1];
    ...
    public void setMakeupType(int type, String path) {
        Log.i("makeup", "type:" + type + " path:" + path);
        if (path == null || type == -1) {
            return;
        }

        if (mMakeupPaths[type] != null &&
                mMakeupPaths[type].equals(path)) {
            // already set the type
            return;
        }
        ...
        mMakeupPackageIds[type] = packageId;
        mMakeupPaths[type] = path;
    }
    ...
    public void removeMakeupByType(int type) {
        if (mMakeupPackageIds[type] != -1) {
            mSTMobileMakeupNative.removeMakeup(mMakeupPackageIds[type]);
            mMakeupPackageIds[type] = -1;
            mMakeupPaths[type] = null;
        }
    }
